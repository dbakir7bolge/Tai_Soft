...
function renderProfile(){const wrap=$(".profile-wrap"),mono=$("#monogram"),img=$("#profilePhoto"),identity=$(".identity");mono.textContent=initials(current.name);if(profileImage){img.src=profileImage;img.hidden=false;img.style.display="block";mono.hidden=true;mono.style.display="none";wrap.hidden=false;wrap.classList.remove("is-empty");identity.classList.remove("no-profile")}else{img.removeAttribute("src");img.hidden=true;img.style.display="none";mono.hidden=true;mono.style.display="none";wrap.hidden=true;wrap.classList.add("is-empty");identity.classList.add("no-profile")}}
function render(){
...
function fillProfilePreview(){const img=$("#profilePreview"),fallback=$("#profilePreviewFallback");fallback.textContent=initials(current.name);if(profileImage){img.src=profileImage;img.hidden=false;img.style.display="block";fallback.hidden=true;fallback.style.display="none"}else{img.removeAttribute("src");img.hidden=true;img.style.display="none";fallback.hidden=false;fallback.style.display="grid"}}
...
async function onProfileSelected(e){const file=e.target.files?.[0];if(!file)return;try{profileImage=await resizeProfile(file);fillProfilePreview();render();toast("Profil resmi eklendi. Kaydet'e basın.")}catch(err){console.warn(err);toast(err.message==="size"?"Görsel 12 MB'den küçük olmalı":"Geçerli bir JPG, PNG veya WEBP seçin")}}
...
$("#removeProfileBtn").addEventListener("click",()=>{profileImage="";$("#profileInput").value="";fillProfilePreview();render();toast("Profil resmi kaldırıldı. Kaydet'e basın.")});
